...
export function generateBenchSubstitutes(teamBase, clubRoster = null, userBench = null) {
  ...
  // Find or fallback for each key position: GK, DEF, MID, ST
  const gkSub = benchList.find((p) => p.role === "GK") || benchList[0] || { role: "GK" };
  const defSub = benchList.find((p) => p.role.includes("B") || p.role === "DEF") || benchList[1] || { role: "CB" };
  const midSub = benchList.find((p) => p.role.includes("M") || p.role === "MID") || benchList[2] || { role: "CM" };
  const stSub = benchList.find((p) => p.role === "ST" || p.role === "FWD" || p.role === "LW" || p.role === "RW") || benchList[3] || { role: "ST" };

  return [
    {
      id: "sub_1",
      number: gkSub.number || 12,
      name: gkSub.name || getRandomCmPlayer("GK").name || "Backup Goalkeeper",
      role: gkSub.role || "GK",
      roleKey: "gk_sub",
      transferValue: gkSub.transferValue || 5.0,
      tier: gkSub.tier || 2,
      promptCapability: gkSub.promptCapability || "GK Sub",
      isYouth: !!gkSub.isYouth,
      prompt: "Backup goalkeeper: focused shot-stopping, solid commanding of the penalty area.",
      stamina: 100,
      tactic: compilePlayerRole("gk", "Backup goalkeeper: alert shot-stopper, reliable hands", teamBase, gkSub.role || "GK"),
      used: false,
    },
    {
      id: "sub_2",
      number: defSub.number || 13,
      name: defSub.name || getRandomCmPlayer("CB").name || "Defensive Stopper",
      role: defSub.role || "CB",
      roleKey: "def_sub",
      transferValue: defSub.transferValue || 5.5,
      tier: defSub.tier || 2,
      promptCapability: defSub.promptCapability || "DEF Sub",
      isYouth: !!defSub.isYouth,
      prompt: "Defensive reinforcement: tight marking, aggressive aerial clearances, hold the backline.",
      stamina: 100,
      tactic: compilePlayerRole("def", "Defensive reinforcement: tight marking, backline stability", teamBase, defSub.role || "CB"),
      used: false,
    },
    {
      id: "sub_3",
      number: midSub.number || 14,
      name: midSub.name || getRandomCmPlayer("CM").name || "Midfield Dynamo",
      role: midSub.role || "CM",
      roleKey: "mid_sub",
      transferValue: midSub.transferValue || 6.0,
      tier: midSub.tier || 2,
      promptCapability: midSub.promptCapability || "MID Sub",
      isYouth: !!midSub.isYouth,
      prompt: "Midfield dynamo: box-to-box energy, retain possession, press ball carriers.",
      stamina: 100,
      tactic: compilePlayerRole("mid2", "Midfield dynamo: ball recovery and transition links", teamBase, midSub.role || "CM"),
      used: false,
    },
    {
      id: "sub_4",
      number: stSub.number || 15,
      name: stSub.name || getRandomCmPlayer("ST").name || "Impact Striker",
      role: stSub.role || "ST",
      roleKey: "st_sub",
      transferValue: stSub.transferValue || 6.5,
      tier: stSub.tier || 2,
      promptCapability: stSub.promptCapability || "ST Sub",
      isYouth: !!stSub.isYouth,
      prompt: "Impact striker: fresh explosive pace, attack the box directly, shoot on sight.",
      stamina: 100,
      tactic: compilePlayerRole("fwd", "Impact striker: fresh pace, shoot on sight", teamBase, stSub.role || "ST"),
      used: false,
    },
  ];
}
